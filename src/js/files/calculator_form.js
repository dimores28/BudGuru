document.addEventListener("DOMContentLoaded", function () { 
    // Перевіряємо чи є простий калькулятор на сторінці
    const simpleCalculator = document.querySelector('.calculator-form:not([data-calculator-type])');
    if (!simpleCalculator) return; // Якщо немає простого калькулятора, виходимо

    // Збираємо всі секції і кнопки тільки для простого калькулятора
    const steps = simpleCalculator.querySelectorAll('.calculator__step');
    const nextButtons = simpleCalculator.querySelectorAll('.calculator__nextstep-btn');
    ...
    function validateStep(stepIndex) {
        ...
        switch(stepIndex) {
            ...
            case 1: // Другий крок - площа
                const rangeInput = currentStepElement.querySelector('#range-input'); // Виправлено ID
    ...
    // Додаємо обробники для кнопок тільки в простому калькуляторі
    ...
    // Додаємо обробники для радіо кнопок і range
    steps.forEach((step, index) => {
        ...
        // Для range input
        const rangeInput = step.querySelector('#range-input'); // Виправлено ID
        if (rangeInput) {
